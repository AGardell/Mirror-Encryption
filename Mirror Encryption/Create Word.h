...
std::pair<int, int> tranverseBoard (std::pair <int, int>& coord) { //depending on starting coordinates, process through the board.
	if (coord.second == 0) {
		return moveRight(coord);
	}
...
void decodeWord(std::string& str) {
	...
	for (char c : str) { //check if alpha, otherwise find the starting location on the board.
		if (!isalpha(c)) {
			std::cout << c;
		}
		else {
